...
class Simulator:
    ...
    def run_single_task(self, task_index: int = 0, wrap_llm_with_logger: bool = False) -> Dict[str, Any]:
        """
        Run a single task immediately and return its result. This helper is
        useful for interactive debugging: it will instantiate an agent for the
        specific task, run the agent workflow and return the output. If
        `wrap_llm_with_logger` is True, the method will wrap the current LLM
        with a simple logger that prints each LLM call and its response.

        Returns a dict with keys: 'task', 'output', 'agent', and optionally
        'llm_calls' when logging is enabled.
        """
        if not self.tasks:
            raise RuntimeError("No tasks loaded. Call set_task_and_groundtruth() first.")
        ...
        # Optionally wrap the LLM to log calls/responses
        llm_to_use = self.llm
        llm_calls = None
        if wrap_llm_with_logger:
            class _LoggingLLM:
                ...
                def __call__(self, messages, model=None, temperature=0.0, max_tokens=500, stop_strs=None, n=1):
                    ...
                    try:
                        ...
                        resp = self._inner(messages, model=model, temperature=temperature, max_tokens=max_tokens, stop_strs=stop_strs, n=n)
                        self.calls.append({"response": resp})
                        # Print compactly for interactive inspection
                        print("LLM request:\n", messages)
                        print("LLM response:\n", resp)
                        return resp
                    except Exception as e:
            ...
            llm_to_use = _LoggingLLM(self.llm)
            llm_calls = llm_to_use.calls

        # Instantiate the agent and run the task synchronously
        if isinstance(llm_to_use, list):
            agent = self.agent_class(llm=llm_to_use[task_index % len(llm_to_use)])
        else:
            agent = self.agent_class(llm=llm_to_use)
